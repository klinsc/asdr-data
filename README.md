...
13. **In order to improve Dataset-6**

    - In Dataset-6, after some experiments, we saw that the big issues that holds back the accuracy metrics is the imbalancing of datasets. We have totally 75 images, with 41 classes, and there are some classes that have low mAP(50:95).
    - From the past experiments, we have selected the best model from our experiments that gives mAP at 0.528 and F1 at 0.67/0.631.
    - Some classes that have mAP(50:95) <=0.5 are 115_3ways_ds, 115_buffer, 115_ds, 115_gs, 22_ds_la_out, 22_ll, BCU, DIM, MU, terminator_double, terminator single, terminator_splicing_kits.
    - The **115_3ways_ds** has 0 mAP, because it have only 4 instances to be trained. When compared to the other classes, it is too few. We can assume that the class might be some faulty classes that were mistaken drawn by a user. Actually the correct drawing and its label should be 115_3ways_ds_w_motor. So we will remove this class from the dataset. Because the user drew it wrong, so it is not a valid class. !!! After looking at the training images, I found that there is only 2 image that has this class, and one of them is the drawing that has 115_3ways_ds_w_motor. But I label it as 115_3ways_ds.
    - The **115_buffer** has 0.22 mAP, because it has only 22 instances to be trained. The reason of this low mAP might came from the same reason. But we will keep this class in the dataset, for it is a valid class.
    - The **115_ds** is a valid class, but when compared with other low mAP classes, its instances is more than the others. For example, if we take a look at the **115_buffer**, it has 22 instances, but its mAP is 0.22, while the **115_ds** has 42 instances, but its mAP is only 0.06. And when we look at their images, we can see that the **115_ds** has more abstract shapes (only lines) than the **115_buffer** (square box).
    - For all of the rest, after I try to solve the problem, I found that the number of training instances and the number from the data set are not the same. For example, the **22_ds_la_out** has 10 instances, but the data set has 645 instances. So I will reannotate the data set and try again.
    -

14. **How to create conda environment with CUDA activated**
    - First, we create a conda environment with python 3.10.12
      - `conda create -n asdr python=3.10.12`
    - Then, we activate the environment
      - `conda activate asdr`
    - Next, we install all CUDA related packages by official pytorch https://pytorch.org/get-started/previous-versions/
      - `conda install pytorch==1.12.1 torchvision==0.13.1 torchaudio==0.12.1 cudatoolkit=11.3 -c pytorch`
    - Then, we install all other packages from YOLOv5 official requirements.txt
      - `pip install -r requirements.txt`
    - Finally, we can train the model with CUDA activated
      - `python train.py --img 1280 --batch 64 --epochs 1000 --data data/asdr.yaml --weights yolov5s.pt --cache`

<!-- region filtering is good for overwatching overall labels => it can be used to group or filter the labels -->
